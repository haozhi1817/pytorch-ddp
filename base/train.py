...
def main(cfg_path):
    ...
    enginer = Base(cfg=cfg, if_training=True)
    train_loader = enginer.train_loader
    ...
    for epoch in range(n_epochs):
        ...
        for batch_id, (batch_imgs, batch_labels) in enumerate(train_loader):
            enginer.train_one_step(
                batch_imgs, batch_labels, current_step=epoch * num_batch + batch_id
            )
        enginer.save_ckpt(epoch)
